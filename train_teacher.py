...
def main(args):
    ...
    base_model = get_model_from_name( model_config, 'ResNet10_l' )
    model_name = args.model_name
    logger.log(("Model: {}".format(model_name)))
    ...
    logger.log("Scheduling LR update model {} time at {}-epoch intervals".format(args.sched_cycles, 
    ...
    logger.log("-" * 50)
    logger.log(
        "[Model] Params={:.2f} MB, FLOPs={:.2f} M ... = {:.2f} G".format(
            param, flop, flop / 1e3
        )
    )
    ...
    logger.log(
        "\n\n***{:s}*** [Post-train] [Teacher model] Test loss = {:.6f}, accuracy@1 = {:.2f}, accuracy@5 = {:.2f}, error@1 = {:.2f}, error@5 = {:.2f}".format(
            time_string(),
            test_loss,
            test_acc1,
            test_acc5,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, help="Folder to save checkpoints and log.", default='./pretrained/')
    ...
    if (args.file_name is None or args.file_name == ""):
        args.file_name = "teacher_seed-{}".format(args.rand_seed)
