import sys, time, torch, random, argparse, json, math, shutil, os
import itertools
...
# get model save location name
def get_model_prefix( args ):
    prefix = './trained/' + m__get_prefix(args) 
    return prefix
...
def get_reg(  args, gate, y_one_hot, s_y_hat, t_y_hat, y, epoch ):
    ...
    if args.penalty == 24: # always true
...
def get_nll_loss_with_hint( args, s_logits, t_logits, s_pred, t_pred, target, gate, y_one_hot, temperature ):
        ...
        if args.use_prob == 50:
            ...
            one_hot = torch.max( one_hot, y_one_hot ) # WHY??
            # acc to paper, one_hot should be the indicator function of y_one_hot being part of one_hot
            # but here one_hot is union of one_hot and y_one_hot 
...
def get_loss( logger, g_data, data, target, model, global_model, gating_model, criterion, args, part, add_gate_loss=False, batch_idx=0, vec_stats=None, vec_s_pred=None, vec_t_pred=None, learn_gate=False, epoch=1  ):
    ...
    if part=='gating':
        ...
        s_ft, s_logits, s_all_ft = model(data)
        if args.use_only_bn == 1: # never reached
           assert(1==2)
        else:
           if args.use_alt_min == 0: # false
               gate, gate_logits = gating_model( s_ft, s_logits, t_ft, t_logits, target, s_all_ft, t_all_ft  )
           else:    
    ...
    if part=='student' and args.use_alt_min==1 and args.b_add_sparsity_alt_min==0: # true
        penalty_loss = 0. * loss
        loss = 0. * loss

    # Auxiliary loss function
    aux_loss = (0. * loss).clone().detach() 
    if args.use_auxiliary == 1:# always true
        aux_loss = args.l_aux * F.cross_entropy(s_logits, target) # 0.1 x
        loss += aux_loss 

    # Add KL Div if asked
    temperature = args.KD_temperature
    if args.use_kl == 1: # always false
        log_student = F.log_softmax(s_logits / temperature, dim=1)
        sof_teacher = F.softmax(t_logits / temperature, dim=1)
        KD_loss = F.kl_div(log_student, sof_teacher, reduction="batchmean") * (
            args.l_kl * temperature * temperature # l_kl is 0
        )
        loss += KD_loss 
...
def main_train_eval_loop( logger, args, model, global_model, routingNet, 
    ...
    g_optimizer = get_optimizer( args.g_lr, args.routing_opt_type, args, routingNet, wd=args.r_wd )

    T = args.KD_temperature
    args.T = T
    args.steps = int(args.epochs // 4)

    g_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(g_optimizer, args.steps)
    s_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(s_optimizer, args.steps)
    t_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(t_optimizer, args.steps)
    ...
    for epoch in range(0, args.init_epochs):
        ...
        logger.log('\t\t LR=' + str(get_mlr(g_scheduler)) + ' init-LR=' + str(get_mlr(init_scheduler)) + \
            ' -- base-LR=' + str(get_mlr(s_scheduler)) + ' -- global-LR=' + str(get_mlr(t_scheduler)))
    ...
    dual_update_step_correction_term( old_s, old_g, new_s, new_g, s_optimizer, g_optimizer, logger, args, train_loader, routingNet, global_model, model, epoch=1 )
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch>1:
            ...
            dual_update_step_correction_term( old_s, old_g, new_s, new_g, s_optimizer, g_optimizer, logger, args, train_loader, routingNet, global_model, model, epoch=epoch )
        ...
        for kt in range(args.kt):
            bg_loss = train_part(logger, train_loader, model, global_model, routingNet, criterion, g_optimizer, epoch, args, s_optimizer, t_optimizer, part='student')

        if args.use_alt_min == 1:
          for kt in range(args.kg):
            train_part(logger, train_loader, model, global_model, routingNet, criterion, g_optimizer, epoch, args, s_optimizer, t_optimizer, part='gating')
        ...
        #     best_state_dict = copy.deepcopy( model.state_dict() )
        #     best_epoch = epoch+1
        val_loss, val_acc1, val_acc5 = evaluate_model( model, val_loader, criterion, args.eval_batch_size )
        is_best = False
        ...
        logger.log('\t\t g\'s LR=' + str(get_mlr(g_scheduler)) + ' init-LR=' + str(get_mlr(init_scheduler)) + \
            ' -- student-LR=' + str(get_mlr(s_scheduler)) + ' -- teacher-LR=' + str(get_mlr(t_scheduler)) )
        logger.log('\t\t Valid evaluation at end of epoch: latest_acc:{:.2f} -- best val acc so far {:.2f}'.format(val_acc1, best_acc))
        ...
        save_checkpoint({
                ...
                'init_optimizer' : init_optimizer.state_dict(),
    ...
    logger.log("\nResult is from best val model of epoch:{}".format(best_epoch))
